...
class PermutationSamplingSTII(Approximator):
    ...
    def __init__(self, n, max_order, random_state = None) -> None:
        super().__init__(
            n=n, max_order=max_order, index="STII", top_order=False, random_state=random_state
        )
        self.iteration_cost = self._compute_iteration_cost()

    def approximate(
        self, budget, game, batch_size = 1
    ) -> InteractionValues:
        ...
        if self.max_order > 1 and budget >= lower_order_cost:
            ...
            warnings.warn(
                "The budget {} is too small to compute the lower order interactions of the STII index, "
                "which requires {} evaluations. Consider increasing the budget.".format(budget, lower_order_cost),
                UserWarning
            )
