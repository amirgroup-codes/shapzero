...
class CoalitionSampler:
    ...
    def __init__(
        ...
        if n_players + 1 != np.size(sampling_weights):  # shape of sampling weights -> sizes 0,...,n
            raise ValueError(
                "{} elements must correspond to {} coalition sizes (including empty subsets)".format(
        ...
        self.adjusted_sampling_weights = None

        # set sample size variables (for border trick)
        self._coalitions_to_compute = None  # coalitions to compute
        self._coalitions_to_sample = None  # coalitions to sample

        # initialize variables to be computed and stored
        self.sampled_coalitions_dict = None  # coal -> count
        self.coalitions_per_size = None  # number of coalitions per size

        # variables accessible through properties
        self._sampled_coalitions_matrix = None  # coalitions
        self._sampled_coalitions_counter = None  # coalitions_counter
