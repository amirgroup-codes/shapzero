...
class Approximator(ABC):
    ...
    def __init__(
        ...
        self.index = index
        self.approximation_index = get_computation_index(index)
        if self.approximation_index not in AVAILABLE_INDICES_FOR_APPROXIMATION:
            raise ValueError(
                "Index {} cannot be approximated. Available indices are {}.".format(
                    self.index, AVAILABLE_INDICES_FOR_APPROXIMATION
                )
            )
        ...
        self.min_order = self.max_order if self.top_order else min_order
        self._grand_coalition_set = set(range(self.n))
        self._grand_coalition_tuple = tuple(range(self.n))
        self._grand_coalition_array = np.arange(self.n + 1, dtype=int)
        self.iteration_cost = 1  # default value, can be overwritten by subclasses
        ...
        self._big_M = 100000000  # large number for sampling weights
