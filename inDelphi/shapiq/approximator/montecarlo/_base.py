...
class MonteCarlo(Approximator):
    ...
    def __init__(
        ...
        if index not in AVAILABLE_INDICES_MONTE_CARLO:
            raise ValueError(
                "Index {} not available for Regression Approximator. Choose from {}.".format(
                    index,
                    AVAILABLE_INDICES_MONTE_CARLO
                )
            )
        if index == "FSII":
            top_order = True
    ...
    def _intersection_stratification(self, interaction) -> np.ndarray:
    ...
    def _svarmiq_routine(self, interaction) -> np.ndarray:
    ...
    def _weight(self, index: str, coalition_size: int, interaction_size: int) -> float:
        ...
        if index == "STII":
           ...
           raise ValueError("The index {} is not supported.".format(index))
