...
class Game(ABC):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.precompute_flag = False  # flag to manually override the precomputed check

        # define storage variables
        self.value_storage = np.zeros(0, dtype=float)
        self.coalition_lookup = {}
        self.n_players = n_players  # if path_to_values is provided, this may be overwritten
    ...
    def precompute(self, coalitions: Optional[np.ndarray] = None) -> None:
        ...
        game_values = self(coalitions_array)  # call the game with the coalitions
    ...
    def load_values(self, path: str, precomputed: bool = False) -> None:
        ...
        if self.n_players is not None and n_players != self.n_players:
            raise ValueError(
                "The number of players in the game ({}) does not match the number "
                "of players in the saved game ({}).".format(self.n_players, n_players)
            )
        self.n_players = int(n_players)
        self.value_storage = data["values"]
        coalition_lookup = transform_array_to_coalitions(data["coalitions"])
    ...
    def __repr__(self) -> str:
        ...
        return (
            "{}({} players, normalize={}, normalization_value={}, precomputed={})".format(
