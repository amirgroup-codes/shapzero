...
class InteractionValues:
    ...
    def __init__(self, 
                 ...
                 n_players: int,
                 min_order: int = 0,
                 interaction_lookup: Optional[Dict[Tuple[int, ...], int]] = None,
                 estimated: bool = True,
    ...
    def __post_init__(self) -> None:
        ...
        if self.index not in ALL_AVAILABLE_INDICES:
            warnings.warn(
                "Index {} is not a valid index as defined in {}. This might lead to unexpected behavior.".format(
                    self.index,
                    ALL_AVAILABLE_INDICES
                ),
                UserWarning
    ...
    def sparsify(self, threshold: float = 1e-3) -> None:
        ...
        interactions_to_remove = set(np.where(np.abs(self.values) < threshold)[0])
    ...
    def get_top_k(
        self, k: int, as_interaction_values=True
    ...
    def __repr__(self) -> str:
        ...
        representation += (
            "    index={}, max_order={}, min_order={}, estimated={}, estimation_budget={},\n"
    ...
    def __add__(self, other: Union["InteractionValues", int, float]) -> "InteractionValues":
        ...
        if isinstance(other, InteractionValues):
            if self.index != other.index:  # different indices
                raise ValueError(
                    "Cannot add InteractionValues with different indices {} and {}.".format(
                        self.index, other.index
                    )
                )
            if (
           ...
           raise TypeError(
                "Cannot add InteractionValues with object of type {}.".format(type(other))
            )
        return InteractionValues(
            values=added_values,
            index=self.index,
