...
class TabularExplainer(Explainer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if index not in AVAILABLE_INDICES:
            raise ValueError("Invalid index `{0}`. Valid indices are {1}.".format(index, AVAILABLE_INDICES))
        ...
        if imputer == "marginal":
            ...
            raise ValueError(
            "Invalid imputer {0}. Must be one of ['marginal', 'conditional'], or a valid Imputer object.".format(imputer)
        )
    ...
    def explain(
        self, x: np.ndarray, budget = None, random_state = None
    ) -> InteractionValues:
        ...
        if budget is None:
            ...
            if budget > 2048:
                warnings.warn(
                    "Using the budget of 2**n_features={0}, which might take long to compute. "
                    "Set the `budget` parameter to suppress this warning.".format(budget)
                )
    ...
    def _init_approximator(
        ...
        try:
            ...
            raise ValueError(
                "Invalid approximator `{0}` or index `{1}`. "
                "Valid configurations are described in {2}.".format(approximator, index, APPROXIMATOR_CONFIGURATIONS)
            )
