...
import sys
sys.path.append('../..')
from gen.shapzero import plot_shap_values, plot_interactions_summary, top_shap_values, top_interactions, plot_interactions
...
plt.savefig('shap_results/inDelphi_1_bp_ins_time.pdf', transparent=True, dpi=300)
...
shap = np.load(f'shap_results/shap_values_{celltype}.npy')[5, :, :]
...
plt.savefig('shap_results/inDelphi_1_bp_ins_shap.pdf', transparent=True, dpi=300)
plt.show()



"""
HEK293 frameshift and 1_bp_ins interaction plots
"""
property = 'frameshift'
...
encoding = {0:'A', 1:'C', 2:'T', 3:'G'}
sequences = [''.join(encoding[num] for num in row) for row in shapzero_sequences]
plot_interactions(ax1, sequences, shapzero_interactions_sequences, top_values=80, x_label='', y_label='Faith-Shap interaction using \n SHAP Zero for \n frameshift frequency', font_size=font_size, markersize=markersize, legend=True, legend_marker_size=legend_marker_size, linewidth=linewidth)
bounds = ax1.get_ylim()
...
sequences = [''.join(encoding[num] for num in row) for row in x_valid]
shap_interactions = np.load(f'shap_results/fsi_{celltype}_{property}.pickle', allow_pickle=True)
plot_interactions(ax2, sequences, shap_interactions, top_values=80, x_label='', y_label='Faith-Shap interaction using \n SHAP-IQ for \n frameshift frequency', y_limits=bounds, font_size=font_size, markersize=markersize, legend=True, legend_marker_size=legend_marker_size, linewidth=linewidth)
bounds = ax1.get_ylim()
...
sequences = [''.join(encoding[num] for num in row) for row in x_valid]
shap_interactions = np.load(f'shap_results/fsi_{celltype}_{property}.pickle', allow_pickle=True)
plot_interactions(ax4, sequences, shap_interactions, top_values=80, x_label='Target sequence position', y_label='Faith-Shap interaction using \n SHAP-IQ for \n 1-bp insertion frequency', y_limits=bounds, font_size=font_size, markersize=markersize, legend=False, linewidth=linewidth)
plt.savefig('shap_results/inDelphi_interaction_plots.pdf', transparent=True, dpi=300)
plt.show()
