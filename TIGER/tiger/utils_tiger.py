...
def common_parser_arguments():
    ...
    # Add qsft arguments to stop parser errors - not needed for tiger
    parser.add_argument("--q", required=False)
    parser.add_argument("--n", required=False)
    parser.add_argument("--b", required=False)
    parser.add_argument("--num_subsample", required=False)
    parser.add_argument("--num_repeat", required=False)
...
def parse_common_arguments(parser: argparse.ArgumentParser, dataset_parameter_override: str = None):
    ...
    if 'off-target' in dataset or 'flow-cytometry' in dataset: # Updated if loop from original code
    ...
    # check if dataset only contains PM guide
...
def load_arguments():
    ...
    args = parse_common_arguments(parser, dataset_parameter_override=dataset_path)
    ...
    data = data.loc[~data.observed_lfc.isna()]
    data = training_validation_split_targets(data, train_ratio=0.9)
    data = data[data['guide_type'] == 'PM'] # Keep only PM sequences

    return data, normalizer, args