...
top_shap_values(sequences, shap_values, top_values=20, filename=f'shap_results/{property}_kernelshap_values')
bounds = ax2.get_ylim()
...
top_shap_values(sequences, shap_zero, top_values=20, filename=f'shap_results/{property}_shapzero_values')
correlation_shap_values(shap_zero, shap)

# Calculate what % of top positive interactions are C-G and in the seed region
top_vals = pd.read_csv(f'shap_results/{property}_shapzero_values.csv')
...
top_vals = pd.read_csv(f'shap_results/{property}_kernelshap_values.csv')
pos_df = top_vals[top_vals['Sign'] == 'Positive']
ratio = pos_df[(pos_df['Position'].between(6, 12)) & (pos_df['Feature'].isin(['C', 'G']))]['Average value'].abs().sum() / pos_df['Average value'].abs().sum()
...
top_interactions(shapzero_sequences, shapzero_interactions_sequences, top_values=20, filename=f'shap_results/{property}_shapzero_fsi')

# Calculate what % of top positive interactions have an A 
top_vals = pd.read_csv(f'shap_results/{property}_shapzero_fsi.csv')
pos_df = top_vals[top_vals['Sign'] == 'Positive']
